 ...
 */
float euclidean_distance(float * d_item, float * n_item, int size) {
    float sqrt_sum = 0;
...
void labeling(vector * v , neural * n, int * data_flow,  match_unit * bmu, params parameters, int isDisplay){
		...
		if(parameters.usage == 1){
			printNeuronAppreciation(*n, parameters, parameters.test, 0.2);
...
float distance(float data_item, float neighbor_item, float learning_rate){
		float distance = pow((data_item - neighbor_item), 2);
		//printf("%lf %lf \n", learning_rate, exp(-(distance / (2 * pow(learning_rate,2) ))));
		return exp(-(distance / (2 * pow(learning_rate,2) )));
}
